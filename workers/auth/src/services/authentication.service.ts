...
import { AuthResults, Organizations } from '../models';
import { AuthDataService } from './auth.data-service';
import { Auth0Service } from './auth0.service';

export class AuthenticationService {
  constructor(private readonly auth0: Auth0Service, private readonly config: AuthConfig, private readonly data: AuthDataService) {}

  async handleCallbackAsync(req: Request): Promise<Headers | null> {
    //
  ...
  async authorizeSiteAsync(req: Request): Promise<Response> {
    ...
    const state = await this.auth0.generateStateParamAsync();
    const url = new URL(req.url);

    await this.data.putStateAsync(state, {});

    return Response.redirect(this.auth0.getLoginRedirectUrl(url.origin, state));
  }

  async authorizeApiAsync(req: Request, performAdminCheck = false): Promise<Response | number> {
    const stateCode = this.getStateCode(req);
    const results: AuthResults = {};

    if (stateCode) {
      results.state = await this.data.getStateAsync(stateCode);

      if (results.state) {
        const org = this.getOrganization(req);

        if (org) {
          results.roles = results.state.organizations?.find((x) => x.organization === org);

          if (performAdminCheck && (results.roles?.roles?.indexOf('admin') ?? -1) === -1) return 401;
        }
      }
    }
    if (results.state) {
  ...
  async getLogoutRedirectAsync(req: Request): Promise<Response> {
    const url = new URL(req.url);
    const state = this.getStateCode(req);

    if (state) await this.data.deleteStateAsync(state);

    return Response.redirect(this.auth0.getLogoutUrl(url.origin));
  }

  async setupAsync(inviteCode: string): Promise<Response | number> {
    const state = await this.auth0.generateStateParamAsync();
  ...
  finishLogout(req: Request): Response | null {
    ...
    if (cookieHeader && cookieHeader.includes(this.config.cookieKey)) {
      ...
      return new Response(LOGOUT_HTML, {
        status: 200,
        headers: {
          'Set-cookie': `${this.config.cookieKey}="";auth0=""; HttpOnly;${secure} SameSite=Lax`,
  ...
  private getOrganization(req: Request): string | null {
  ...
  private validateToken(token: { iss: string; aud: string; exp: number; iat: number }) {
    try {
      ...
      if (token.aud !== this.config.authClientId) {
        throw new Error(`Token aud value (${token.aud}) doesn't match AUTH0_CLIENT_ID (${this.config.authClientId})`);
      }
