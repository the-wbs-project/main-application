...
export class WbsDisciplineNodeTransformer {
  ...
  async runAsync(projectId: string): Promise<WbsNodeView[]> {
    let [dList, pList, resourceList, project, projectNodes] = await Promise.all([
    ...
    return this.fromPhaseFirst(projectNodes, disciplines, phases, resources);
  }

  private fromPhaseFirst(projectNodes: WbsNode[], disciplines: ListItem[], phases: ListItem[], resources: ResourceService): WbsNodeView[] {
    ...
    for (let i = 0; i < disciplines.length; i++) {
      ...
      for (const p of phases) {
        ...
        const children = this.getPhaseChildren(d.id, p.id, p.id, pLevel, projectNodes);
  ...
  private getSortedChildren(parentId: string, list: WbsNode[]): WbsNode[] {
    return list.filter((x) => !x.removed && x.parentId === parentId).sort(WbsNodeService.sort);
    //&& (x.disciplineIds ?? []).indexOf(disciplineId) > -1
  }

  private getPhaseChildren(disciplineId: string, phaseId: string, parentId: string, parentLevel: number[], list: WbsNode[]): WbsNodeView[] {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      const myChildren = this.getPhaseChildren(disciplineId, phaseId, child.id, childLevel, list);

      const hasDiscipline = (node.disciplines ?? [])?.indexOf(disciplineId) > -1;
