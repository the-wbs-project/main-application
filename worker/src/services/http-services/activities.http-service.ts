...
export class ActivitiesHttpService {
  static async getAsync(ctx: Context): Promise<Response> {
    try {
      ...
      const memberRoles = await ctx.var.data.members.getMemberRolesAsync(owner);
      const userIds = Object.keys(memberRoles);
      const users = (await ctx.var.data.users.getUsersAsync(userIds)).map((x) => Transformers.user.toViewModel(x, 'organization'));
