...
import { Auth0Service, DataDogService, DataServiceFactory, Fetcher, JobLogger, QueueService, UserSearchDataService } from './services';
...
export const APP_QUEUE = async (batch: MessageBatch, env: Env, ctx: ExecutionContext): Promise<void> => {
  ...
  const data = new DataServiceFactory(new Auth0Service(env, fetcher, logger), env.KV_DATA, ctx);
