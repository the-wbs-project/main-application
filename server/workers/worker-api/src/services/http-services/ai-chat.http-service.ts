...
export class AiChatHttpService {
  static async getAsync(ctx: Context): Promise<Response> {
    try {
      ...
      ctx.var.logger.trackException('An error occured trying to get AI chat history.', <Error>e);
  ...
  static async putAsync(ctx: Context): Promise<Response> {
    try {
      ...
      await ctx.var.data.aiChat.putAsync(model, userId, history);

      return ctx.newResponse(null, 202);
    } catch (e) {
      ctx.var.logger.trackException('An error occured trying to save AI chat history.', <Error>e);
  ...
  static async deleteAsync(ctx: Context): Promise<Response> {
    try {
      ...
      ctx.var.logger.trackException('An error occured trying to delete AI chat history.', <Error>e);
