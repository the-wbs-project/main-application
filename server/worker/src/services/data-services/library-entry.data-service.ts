...
export class LibraryEntryDataService extends BaseDataService {
  ...
  async getTasksByVersionAsync(owner: string, entry: string, version: number, visibility: string): Promise<LibraryEntryNode[] | undefined> {
    if (visibility === 'public') {
      return await this.getPublicTasksAsync(owner, entry, version);
    }
    const memberships = await this.ctx.var.data.memberships.getMembershipsAsync(this.ctx.var.idToken.userId, false);
    const isMember = memberships.some((m) => m.name === owner);
  ...
  private async getPublicTasksAsync(owner: string, entry: string, version: number): Promise<LibraryEntryNode[] | undefined> {
