...
export class ThemeService implements OnDestroy {
  ...
  get kendouiStylesheetHtmlElement() {
    if (!this._kendouiStylesheetHtmlElement) {
      ...
      this.renderer.setAttribute(
        this._kendouiStylesheetHtmlElement,
        'rel',
        'stylesheet'
      );
      const headHtmlElement: HTMLHeadElement =
  ...
  isSystemDark(): boolean {
    const result: boolean = this.document.defaultView?.matchMedia(
  ...
  private setKendoUiControlsMode(stylesheetFilePath: string): void {
    this.renderer.setProperty(
      this.kendouiStylesheetHtmlElement,
      'href',
      stylesheetFilePath
    );
  }
  ...
  private setUserDefinedMode(darkMode: boolean): void {
    this.setMode(darkMode);
    this.stopSystemModeSynchronization();
