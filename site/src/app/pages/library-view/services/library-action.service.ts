...
import { MenuService, NavigationService } from '@wbs/core/services';
...
export class LibraryActionService {
        ...
        items.push({
          ...
          items: [
            ...drafts.map((v) => ({
              ...
              route: this.url(version.ownerId, version.recordId, v.version),
            })),
          ],
        });
      }
  ...
  private url(owner: string, recordId: string, version: number): string[] {
    return this.navigation.getLibraryEntryUrl(owner, recordId, version);
  }
